...
const OrderArticles = () => {
  ...
  const fetchArticles = async () => {
    try {
      setLoading(true);
      const response = await api.get('/content/articles?limit=100');
  ...
  const handleSave = async () => {
    try {
      ...
      await api.put('/content/articles/order', {
        items: itemsWithOrder
      });

      setSuccess('Article order saved successfully');
      setHasChanges(false);
      ...
      setError('Failed to save article order');
    } finally {
      setSaving(false);
    }
  };
    ...
    <Box sx={{ p: 3 }}>
      ...
      <Paper sx={{ p: 2 }}>
        <Typography variant="body2" color="text.secondary" sx={{ mb: 2 }}>
          Drag and drop articles to reorder them. This order will be reflected on the public listing page.
        </Typography>
