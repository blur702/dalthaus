            ...
            container.innerHTML = data.articles.map(article => `
                <article class="article-card">
                    ...
                    <div class="article-content">
                        ...
                        <a href="article.html?id=${article.slug}" class="read-more">
                            Read the article
                        </a>
                    </div>
                </article>
        ...
        if (data.photoBooks && data.photoBooks.length > 0) {
            container.innerHTML = data.photoBooks.map(book => `
                <article class="article-card">
                    ...
                    <div class="article-content">
                        <h2 class="article-title">
                            <a href="photobook.html?id=${book.slug}">${book.title}</a>
                        </h2>
                        <p class="article-excerpt">
                            ${truncateText(stripHtml(book.body || ''))}
                        </p>
                        <a href="photobook.html?id=${book.slug}" class="read-more">
                            View photo book
                        </a>
                    </div>
                </article>
    ...
    try {
        ...
        function buildPageTree(pages) {
            return pages.map(page => `
                <li>
                    <a href="page.html?id=${page.slug}">${page.title}</a>
...
async function loadPhotoBook() {
    ...
    try {
        const response = await fetch(`${API_BASE_URL}/photobooks/${bookId}`);
